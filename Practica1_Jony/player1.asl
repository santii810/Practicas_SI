...
//movimiento(moverDesdeEnDireccion(pos(0,0),up)).
correcto(moverDesdeEnDireccion(pos(0,0),up)).
/* Initial goals */
...
+mueve[source(judge)]: correcto(moverDesdeEnDireccion(pos(X,Y),Dir))<- 
	...
	if(Dir=up){-+movimiento(moverDesdeEnDireccion(pos(X,Y+1),D));}
	if(Dir=down){-+movimiento(moverDesdeEnDireccion(pos(X,Y-1),D));}
	if(Dir=left){-+movimiento(moverDesdeEnDireccion(pos(X-1,Y),D));}
	if(Dir=right){-+movimiento(moverDesdeEnDireccion(pos(X+1,Y),D));}
