...
+!agrupacion:grupoT(X,Y,C,A,B) <-
	!calcularPuntos(X,Y,C).
+!agrupacion:grupoCuadrado(X,Y,C,A,B) <-
	!calcularPuntos(X,Y,C).
+!agrupacion:grupo4(X,Y,C,A,B) <-
	!calcularPuntos(X,Y,C).
+!agrupacion:grupo3(X,Y,C,A,B) <-
	!calcularPuntos(X,Y,C).

//Grupo de 5
+!calcularPuntos(X,Y,C): grupo5(X,Y-1,C,X,Y) & not igualColor(X,Y-1,C,C) & not esObstaculo(X,Y-1).	
+!calcularPuntos(X,Y,C): grupo5(X,Y+1,C,X,Y) & not igualColor(X,Y-1,C,C) & not esObstaculo(X,Y-1).
...
+buscarJugadas: size(N) & level(L) & L < 3 <-
	...
	for(.range(X,0,N-1)){
		for(.range(Y,0,N-1)){
			if(datos(X,Y,C) & not esObstaculo(X,Y)){
